...
struct CountryDetailScreen: View {
    ...
    @StateObject var countrySummary = CountryDetailsViewModel()
    let country: Country
                ...
                List {
                    Section(header: Text("Lifetime Stats")) {
                        Text(totalCase.date, style: .date)
                            .font(.headline)
                        CasePieChart(totalCount: .init(title: country.iso.flag, confirmed: totalCase.confirmed, death: totalCase.deaths, recovered: totalCase.recovered), date: totalCase.date)
...
//        CountryDetailScreen(country: .init(id: "", name: "", iso: ""))
